...
export const HowItWorks: React.FC = () => {
  ...
  return (
    <section className="py-24 bg-black/50 relative overflow-hidden" id="solutions">
      {/* Background Decor */}
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-hayrok-border to-transparent"></div>
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            {steps.map((step, index) => (
              <div 
                ...
                className={`p-6 rounded-xl border cursor-pointer transition-all duration-300 ${
                  activeStep === index 
                    ? 'bg-hayrok-panel border-hayrok-orange shadow-[0_0_30px_-10px_rgba(255,107,0,0.2)]' 
                    : 'bg-transparent border-transparent hover:bg-hayrok-panel/50 hover:border-hayrok-border'
                }`}
              >
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg mt-1 transition-colors ${
                    activeStep === index ? 'bg-hayrok-orange text-white' : 'bg-slate-800 text-slate-400'
                  }`}>
                    <step.icon size={20} />
                  </div>
                  <div>
          ...
          <div className="relative h-[500px] glass-panel rounded-2xl border border-hayrok-border overflow-hidden flex items-center justify-center p-8">
            <div className="absolute inset-0 bg-gradient-to-br from-hayrok-orange/5 to-transparent"></div>
              ...
              >
                {activeStep === 0 && (
                   <div className="relative aspect-square">
                      ...
                      {/* Floating Assets */}
                      <motion.div animate={{ x: [0, 20, 0], y: [0, -20, 0] }} transition={{ duration: 4, repeat: Infinity }} className="absolute top-1/4 left-1/4 bg-slate-800 p-2 rounded border border-slate-700 text-xs flex items-center gap-2">
                        <Server size={12} className="text-blue-400"/> API Gateway
                      </motion.div>
                      <motion.div animate={{ x: [0, -15, 0], y: [0, 15, 0] }} transition={{ duration: 5, repeat: Infinity }} className="absolute bottom-1/3 right-1/4 bg-slate-800 p-2 rounded border border-slate-700 text-xs flex items-center gap-2">
                ...
                {activeStep === 1 && (
                  <div className="bg-slate-900 rounded-lg p-4 font-mono text-xs border border-hayrok-border shadow-2xl">
                ...
                {activeStep === 3 && (
                   <div className="bg-white text-black rounded-lg p-6 shadow-2xl">
                      ...
                      <button className="w-full py-2 bg-hayrok-orange text-white rounded font-bold text-sm">
                         View Full Report
                      </button>
                   </div>
                )}
